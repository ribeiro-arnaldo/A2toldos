...
export const gerarOrcamentoPDF = (orcamento, usuarioLogado) => {
  ...
  doc.text(
    `EMAIL: ${orcamento.email_cliente || "Não informado"}`,
    110,
    currentY,
  );
  currentY += 7;
  const docFormatado = formatarDocumento(
    orcamento.cliente_documento,
    orcamento.cliente_tipo_pessoa,
  );
  doc.text(
    `${orcamento.cliente_tipo_pessoa === "FISICA" ? "CPF" : "CNPJ"}: ${docFormatado}`,
    margin + 5,
    currentY,
  );
  doc.text(
    `TELEFONE: ${formatarTelefone(orcamento.telefone_cliente) || "Não informado"}`,
    110,
    currentY,
  );
  currentY += 7;
  ...
  const espacoNecessarioParaRodape = 70; // Espaço em milímetros para as caixas e assinatura

  // Verifica se o rodapé cabe na folha atual
  if (currentY + espacoNecessarioParaRodape > pageHeight) {
    doc.addPage();
    currentY = 20;
  } else {
    currentY += 35; // Um respiro elegante de 35mm
  }

  const alturaCaixas = 30;
  ...
  doc.rect(obsX, obsY, obsWidth, alturaCaixas);
  ...
  doc.setFontSize(10);
  doc.setFont("helvetica", "normal");
  doc.text("Assinatura do Cliente", pageWidth / 2, finalY - 25, {
    align: "center",
  });
