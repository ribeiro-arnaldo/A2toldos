...
import { useParams, Link, useNavigate } from "react-router-dom"; // 1. Importamos o useNavigate
import {
  FiFileText,
  FiArrowLeft,
  FiGrid,
...
const OrcamentoDetailPage = () => {
  ...
  // 3. AQUI ESTÁ A ÚNICA ALTERAÇÃO NA LÓGICA
  const handleStatusUpdate = async () => {
    setLoadingStatus(true);
    try {
      await api.patch(`/orcamentos/${id}/status`, { status: novoStatus });
      setOrcamento((prevState) => ({ ...prevState, status: novoStatus }));
      toast.success("Status atualizado com sucesso!");

      // Adiciona um pequeno atraso e depois navega para a lista forçando o refresh
      setTimeout(() => {
        navigate("/orcamentos", { state: { refresh: true } });
      }, 1000); // 1 segundo de atraso
    } catch (error) {
      toast.error("Falha ao atualizar o status.");
      setLoadingStatus(false); // Reabilita o botão em caso de erro
    }
  };
