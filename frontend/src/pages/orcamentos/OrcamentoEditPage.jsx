...
const OrcamentoEditPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // AQUI ESTÁ A MUDANÇA
      // Redireciona para a lista principal com o sinal para recarregar os dados
      navigate("/orcamentos", { state: { refresh: true } });
    } catch (error) {
      const errorMessage =
        error.response?.data?.erro || "Ocorreu um erro desconhecido.";
  ...
  if (loading) {
    return (
      <div className="p-6 text-center">A carregar orçamento para edição...</div>
    );
  }
