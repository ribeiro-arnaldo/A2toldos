...
const ClienteEditPage = () => {
  ...
  // Memória para guardar os dados do formulário
  ...
  // Quando a página carrega, busca os dados atuais do cliente para preencher o formulário
  useEffect(() => {
    const fetchCliente = async () => {
      try {
        const response = await api.get(`/clientes/${id}`);
        // Formata a data para o formato AAAA-MM-DD que o input type="date" espera
  ...
  const handleChange = (e) => {
    ...
    // ... (lógica de máscara e filtro em tempo real, igual ao formulário de criação)
    if (name === 'nome') {
  ...
  const validateForm = () => {
    // ... (lógica de validação, igual ao formulário de criação)
    const newErrors = {};
  ...
  const handleSubmit = async (e) => {
    ...
    setErrors({});
    try {
      // A grande diferença: usamos o método PUT e passamos o ID do cliente
  ...
  return (
    <div className="p-6">
      ...
      {/* O formulário é visualmente idêntico ao de criação */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input type="text" name="documento" id="documento" value={formData.documento} onChange={handleChange} required 
                   maxLength={formData.tipo_pessoa === 'FISICA' ? 11 : 14}
