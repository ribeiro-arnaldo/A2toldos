...
import { FiUsers, FiPlus, FiSearch, FiRotateCw, FiInfo, FiEdit, FiTrash2 } from "react-icons/fi";
...
const ClientesPage = ({ clientes, setClientes, filtros, setFiltros, buscaRealizada, setBuscaRealizada }) => {
  ...
  const formatarTelefone = (tel) => {
    ...
    if (telLimpo.length === 10) return telLimpo.replace(/(\d{2})(\d{4})(\d{4})/, '($1) $2-$3');
    return telLimpo;
  };
  ...
  return (
    <div>
      ...
      {/* Formulário de Busca com o Estilo Correto */}
      <div className="mb-6 p-4 bg-white rounded-lg shadow">
        <form onSubmit={handleFormSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <div className="md:col-span-1">
              <label htmlFor="tipoFiltro" className="block text-sm font-medium text-gray-700">Filtrar por</label>
              <select id="tipoFiltro" name="tipo" value={filtros.tipo} onChange={handleInputChange} className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-yellow focus:border-brand-yellow">
            ...
            <div className="md:col-span-2">
              <label htmlFor="termoBusca" className="block text-sm font-medium text-gray-700">Termo de Busca</label>
              <input type="text" id="termoBusca" name="termo" value={filtros.termo} onChange={handleInputChange} className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-yellow focus:border-brand-yellow" placeholder={`Digite o ${filtros.tipo}...`} />
            </div>
          </div>
          <div className="mt-4 flex justify-end space-x-4">
            <button type="button" onClick={handleClearFilters} className="bg-gray-200 text-gray-800 font-bold py-2 px-4 rounded-lg flex items-center hover:bg-gray-300 transition-colors"><FiRotateCw className="mr-2" /> Limpar</button>
      ...
      {!buscaRealizada ? (
        <div className="text-center p-12 bg-white rounded-lg shadow">
          ...
          <h3 className="text-lg font-semibold text-gray-700">Faça uma busca para ver os clientes.</h3>
          <p className="text-gray-500">Utilize os filtros acima para encontrar um cliente específico.</p>
        </div>
      ) : loading ? (
        <div className="text-center p-12 bg-white rounded-lg shadow">Carregando...</div>
      ) : clientes.length === 0 ? (
        <div className="text-center p-12 bg-white rounded-lg shadow">Nenhum cliente encontrado.</div>
      ) : (
        <>
          {/* Tabela para Desktop */}
              ...
              <tbody>
                {clientes.map((cliente) => (
                  <tr key={cliente.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="p-4"><div className="flex items-center justify-center space-x-3"><Link to={`/clientes/${cliente.id}/editar`} title="Editar" className="text-blue-600 hover:text-blue-800"><FiEdit size={18} /></Link><button onClick={() => handleDeleteClick(cliente)} title="Apagar" className="text-red-600 hover:text-red-800"><FiTrash2 size={18} /></button></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {clientes.map(cliente => (
              <div key={cliente.id} className="bg-white p-4 rounded-lg shadow">
                <div className="flex justify-between items-start">
                  <Link to={`/clientes/${cliente.id}`} className="font-bold text-brand-blue text-lg">{cliente.nome}</Link>
                  <div className="flex items-center space-x-3">
                    <Link to={`/clientes/${cliente.id}/editar`} title="Editar" className="text-blue-600"><FiEdit size={18} /></Link>
                    <button onClick={() => handleDeleteClick(cliente)} title="Apagar" className="text-red-600"><FiTrash2 size={18} /></button>
                  </div>
                </div>
                <div className="mt-2 text-sm text-gray-600 space-y-1">
                  <p><strong>Documento:</strong> {formatarDocumento(cliente.documento, cliente.tipo_pessoa)}</p>
          ...
          <Pagination pagina={dadosPaginacao.pagina} limite={dadosPaginacao.limite} total={dadosPaginacao.total} onPageChange={handleSearch} />
        </>
      )}
      <ConfirmationModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onConfirm={confirmDelete} title="Confirmar Exclusão" message={`Tem a certeza de que deseja apagar o cliente "${clienteParaApagar?.nome}"?`} />
    </div>
  );
};
