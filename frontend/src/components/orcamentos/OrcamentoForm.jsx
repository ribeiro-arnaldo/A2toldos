...
const OrcamentoForm = ({ formData, setFormData, errors }) => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      try {
        const response = await api.get(
          `/clientes?nome=${termoBuscaCliente}&limite=10`
        );
  ...
  const handleItemChange = useCallback(
    (index, event) => {
      ...
      novosItens[index].subtotal =
        largura > 0 && comprimento > 0 && preco_m2 > 0
  ...
  const removeItem = useCallback(
    (index) => {
      if (itens.length <= 1) return; // Não permite remover o último item
      const novosItens = [...itens];
  ...
  const formInputStyle =
    "mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-yellow focus:border-brand-yellow";
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="relative">
            ...
            {clienteSelecionado && (
              <button
                ...
                className="absolute top-7 right-2 p-1 text-gray-500 hover:text-red-600"
              >
                <FiXCircle />
              </button>
            )}
              ...
              <ul className="absolute z-10 w-full bg-white border border-gray-300 rounded-md mt-1 shadow-lg max-h-60 overflow-y-auto">
                {sugestoesClientes.map((cliente) => (
                  <li
                    ...
                    className="px-4 py-2 hover:bg-brand-yellow cursor-pointer"
                  >
                    {cliente.nome}
                  </li>
                ))}
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ex: Cobertura da área da piscina"
            />
            {errors.descricao && (
          ...
          <div>
            ...
            <input
              ...
              className={`${formInputStyle} ${
                errors.prazo_entrega ? "border-red-500" : ""
              }`}
            />
      ...
      <div className="p-4 border rounded-lg">
        ...
        <div className="space-y-4">
          {itens.map((item, index) => (
            <div
              key={item.id || index}
              className="grid grid-cols-12 gap-x-4 gap-y-2 items-start p-3 border-b last:border-b-0"
            >
              ...
              <div className="col-span-12 md:col-span-6 grid grid-cols-3 gap-x-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleItemChange(index, e)}
                    className={`${formInputStyle} ${
                      errors.itens?.[index]?.largura ? "border-red-500" : ""
                    }`}
                    placeholder="4.50"
                  />
                  {errors.itens?.[index]?.largura && (
                    <p className="mt-1 text-sm text-red-600">
                      {errors.itens[index].largura}
                    </p>
                  )}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleItemChange(index, e)}
                    className={`${formInputStyle} ${
                      errors.itens?.[index]?.comprimento ? "border-red-500" : ""
                    }`}
                    placeholder="3.00"
                  />
                  {errors.itens?.[index]?.comprimento && (
                    <p className="mt-1 text-sm text-red-600">
                      {errors.itens[index].comprimento}
                    </p>
                  )}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleItemChange(index, e)}
                    className={`${formInputStyle} ${
                      errors.itens?.[index]?.preco_m2 ? "border-red-500" : ""
                    }`}
                    placeholder="250.00"
                  />
                  {errors.itens?.[index]?.preco_m2 && (
                    <p className="mt-1 text-sm text-red-600">
                      {errors.itens[index].preco_m2}
                    </p>
                  )}
              ...
              <div className="col-span-12 md:col-span-2 flex items-end justify-between">
                <div className="w-full">
                  ...
                  <div className="mt-1 px-3 py-2 h-[42px] flex items-center bg-gray-100 rounded-md font-semibold">
                ...
                <button
                  ...
                  className="text-red-500 hover:text-red-700 disabled:opacity-50 disabled:cursor-not-allowed ml-2 self-end mb-2"
                >
                  <FiTrash2 size={20} />
                </button>
              </div>
        ...
        <div className="mt-4">
          <button
            type="button"
            onClick={addItem}
            className="flex items-center text-sm font-semibold text-brand-blue hover:text-opacity-80"
          >
