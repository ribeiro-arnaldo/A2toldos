...
router.post('/', (req, res) => {
  ...
  db.serialize(() => {
    ...
    const orcamentoQuery = `
      INSERT INTO orcamentos (cliente_id, descricao, valor_total, data_orcamento)
      VALUES (?, ?, ?, ?)
    `;
    db.run(orcamentoQuery, [cliente_id, descricao, valor_total_calculado, data_orcamento], function(err) {
      ...
      const itemQuery = `
        INSERT INTO itens_orcamento (orcamento_id, descricao_item, largura, comprimento, preco_m2, valor_item)
        VALUES (?, ?, ?, ?, ?, ?)
      `;
...
// Rota GET (TODOS): Listar todos os orçamentos (visão simplificada)
router.get('/', (req, res) => {
  const query = `
    SELECT
      o.id,
